...
def main():
    ...
    Flight_Distance = st.text_input('Enter the Distance travelled by the Customer')
    Seat_Comfort = st.text_input('Enter the rating for Seat Comfort in the Flight : (O to 5)')
    Departure_OR_Arrival_Time_Convenient = st.text_input('Enter the rating for Departure/Arrival time convenient: (O to 5)')
    Food_and_Drink = st.text_input('Enter the rating for Food and Beverages: (O to 5)')
    Gate_Location = st.text_input('Enter the rating for Gate Location: (O to 5)')
    Inflight_Wifi_Service = st.text_input('Enter the rating for Inflight wifi service: (O to 5)')
    Inflight_Entertainment = st.text_input('Enter the rating for Inflight entertainment: (O to 5)')
    Online_Support = st.text_input('Enter the rating for Online support: (O to 5)')
    Ease_of_Online_Booking = st.text_input('Enter the rating for Ease of Online Booking: (O to 5)')
    Onboard_Service = st.text_input('Enter the rating for Onboard Services: (O to 5)')
    Legroom_Service = st.text_input('Enter the rating for Leg room service: (O to 5)')
    Baggage_Handling = st.text_input('Enter the rating for Baggage Handling: (O to 5)')
    Checkin_Service = st.text_input('Enter the rating for Checkin Service: (O to 5)')
    Cleanliness = st.text_input('Enter the rating for Cleanliness in the Flight: (O to 5)')
    Online_Boarding = st.text_input('Enter the rating for Online boarding: (O to 5)')
    Departure_Delay_in_Minutes = st.text_input('Enter the Departure Delay in minutes')
    Arrival_Delay_in_Minutes = st.text_input('Enter the Arrival Delay in minutes')
